...
def pr_score(model):
    ...
    for persone_id in os.listdir("data/images"):
        f = os.path.join("data/images", persone_id)
        ...
        if os.path.isdir(f):
            for image_file in os.listdir(f):
                ...
                if (image_file.endswith(".png")):
                    image_c += 1
                    image_code = image_file[:-4]

                    # Get a list of iou-s for all predicted boxes
                    iou = yolo_iou(persone_id, image_code, model)
                    ...
                    print(f"socre: {iou}; {image_c}", end="\r")
    ...
    with open("res/yolo_precisionn_recall.txt", "w") as pr:
        pr.write(str(precisions) + "\n")
        pr.write(str(recalls))

    print()

    with open("res/jv_res.txt", "a") as f:
